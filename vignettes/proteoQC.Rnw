...
\Rfunction{proteoQCpipe} function, providing appropriate input parameters,
...
\Rfunction{?proteoQCpipe} for a more in-depth description of all its
arguments.

<<run, eval=FALSE, tidy=FALSE>>=
qcres <- proteoQCpipe(spectralist = design,
                  fasta = fas, 
                  outdir = "./qc",
                  miss  = 0,
...
The \Rfunction{proteoQCpipe} function will run each mgf input file
...
the \Rfunction{proteoQCpipe} function above or the directory storing the
QC data, as defined as parameter to the \Rfunction{proteoQCpipe}.
...
as passed to \Rfunction{proteoQCpipe}.
